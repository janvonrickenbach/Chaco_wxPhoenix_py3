...
class WaterfallRenderer(LinePlot):
    ...
    def _gather_points(self):
        if not self._cache_valid:
            ...
            numindex = len(index)
            if numindex == 0 or all(len(v) == 0
                                    for v in values) or all(numindex != len(v)
                                                            for v in values):
...
def _create_plot_component(obj):
    ...
    spec_renderer = obj.spectrum_plot.plot(
        ("frequency", "amplitude"), name="Spectrum", color="red")[0]
    ...
    p = WaterfallRenderer(
        ...
        value_mapper=LinearMapper(range=DataRange1D(
            low=0, high=SPECTROGRAM_LENGTH)),
        y2_mapper=LinearMapper(
            low_pos=0, high_pos=8, range=DataRange1D(
                low=0, high=15)), )
    spectrogram_plot = p
