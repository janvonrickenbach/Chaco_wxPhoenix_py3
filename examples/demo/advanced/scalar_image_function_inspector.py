...
class Model(HasTraits):
    ...
    traits_view = View(
        Group(
            ...
            HGroup(
                Item(
                    'npts_x', label="Number X Points"),
                Item(
                    'npts_y', label="Number Y Points")),
            HGroup(
                Item(
                    'min_x', label="Min X value"),
                Item(
                    'max_x', label="Max X value")),
            HGroup(
                Item(
                    'min_y', label="Min Y value"),
                Item(
                    'max_y', label="Max Y value"))),
        buttons=["OK", "Cancel"])
...
class PlotUI(HasTraits):
    ...
    traits_view = View(
        Group(UItem(
            'container', editor=ComponentEditor(size=(800, 600)))),
        resizable=True)
    ...
    def create_plot(self):
        ...
        self.polyplot.tools.append(
            PanTool(
                self.polyplot, constrain_key="shift"))
        self.polyplot.overlays.append(
            ZoomTool(
                component=self.polyplot, tool_mode="box", always_on=False))
        self.polyplot.overlays.append(
            LineInspector(
                component=self.polyplot,
                axis='index_x',
...
class TimerController(HasTraits):
    ...
    motions = ((1, 1), (1, 2), (1, 3), (2, 1), (3, 1), (3, 2), (2, 3), (1, -1),
               (1, -2), (1, -3), (2, -1), (3, -1), (3, -2), (2, -2))
...
class ModelView(HasTraits):
    ...
    traits_view = View(
        ...
        HGroup(
            UItem(
                'edit_model', enabled_when='not animated'),
            UItem('edit_view'),
            Item('animated'),
            Item(
                'animate_left',
            ...
            Item(
                'animate_right', enabled_when='animated', label='Right'),
            Item(
                'animate_top', enabled_when='animated', label='Top'),
            Item(
                'animate_bottom', enabled_when='animated', label='Bottom'),
            spring),
