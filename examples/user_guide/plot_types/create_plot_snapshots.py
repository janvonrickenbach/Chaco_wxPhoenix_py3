...
def get_cmap_scatter_plot():
    ...
    color_mapper = dc.reverse(dc.RdYlGn)(DataRange1D(
        low=nox.min(), high=nox.max()))
...
def get_errorbar_plot():
    ...
    y = 3.2 * x**2 + 4.0
    y_with_noise = (y[None, :] + scipy.stats.norm(
        loc=0, scale=2.8).rvs((10, 1)))
...
def get_image_from_file():
    ...
    index_mapper = GridMapper(range=DataRange2D(
        low=(0, 0), high=(w - 1, h - 1)))
