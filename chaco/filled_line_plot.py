...
class FilledLinePlot(PolygonPlot):
    ...
    def _render(self, gc, points):
        ...
        render_lines = render_method_dict.get(self.render_style,
                                              LinePlot._render_normal)

        if self.fill_direction == 'down':
            ox, oy = self.map_screen([[0, 0]])[0]
        else:
            ox, oy = self.map_screen(
                [[self.x_mapper.range.high, self.y_mapper.range.high]])[0]
        ...
        with gc:
            ...
            if (not (len(edge_col) == 4 and
                     edge_col[-1] == 0)) and edge_col != face_col:
