...
__all__ = [
    'NullFormatter', 'BasicFormatter', 'IntegerFormatter', 'OffsetFormatter',
    'TimeFormatter', 'strftimeEx'
]
...
class OffsetFormatter(BasicFormatter):
    ...
    def estimate_width(self,
        ...
        start, mid, end = list(
            map(len, self.format([start, (start + end) / 2.0, end])))
        if est_ticks > 2:
...
class TimeFormatter(object):
    ...
    format_order = [
        'microseconds', 'milliseconds', 'seconds', 'minsec', 'minutes',
    ...
    def format(self,
        ...
        if char_width:
            ...
            good_formats = array(formats)[widths * len(ticks) < fill_ratio *
                                          char_width]
        ...
        for t in ticks:
            ...
            while tm[time_tuple_ndx_for_resol[self.format_order[
                    next_ndx]]] == 0:
                next_ndx += 1
                ...
                if resol in ("minsec", "hourmin") and not hybrid_handled:
                    if (resol == "minsec" and tm.tm_min == 0 and tm.tm_sec != 0) or \
                        ...
                        next_format = self.formats[self.format_order[
                            resol_ndx - 1]][1][0]
