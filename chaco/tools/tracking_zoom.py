...
class TrackingZoom(ZoomTool):
    ...
    def normal_mouse_wheel(self, event):
        ...
        if self.enable_wheel and event.mouse_wheel != 0:
            ...
            # in the new range.
            c = self.component
            low_pt, high_pt = self._map_coordinate_box((c.x, c.y),
                                                       (c.x2, c.y2))
            mouse_pos = (c.x_mapper.map_data(event.x),
                         c.y_mapper.map_data(event.y))

            if self.tool_mode == "range":
                datarange_list = [(self._determine_axis(), self._get_mapper()
                                   .range)]
            else:
            ...
            for ndx, datarange in datarange_list:
                ...
                if datarange.default_state == 'low_track':
                    hi = max([
                        source.get_bounds()[1] for source in datarange.sources
                    ])
                    ...
                    lo = min([
                        source.get_bounds()[0] for source in datarange.sources
                    ])
