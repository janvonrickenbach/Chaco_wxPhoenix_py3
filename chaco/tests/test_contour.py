...
class TestContour(unittest.TestCase):
    def test_contour_trace_levels_no_mask(self):
        ...
        data = np.array([[0, 0, 1, 2], [0, 1, 2, 3], [1, 2, 0, 3],
                         [2, 3, 3, 3]])
        ...
        self._check_level(levels[0], [1.0, 1.0, 0.0, 0.0],
                          [10.0, 10.0, 20.0, 20.0])
        self._check_level(levels[1], [2.0, 2.0, 2.0, 2.0, 2.0],
                          [30.0, 30.0, 30.0, 30.0, 30.0])

        levels = c.trace(1.0)
        self.assertEqual(len(levels), 2)
        self._check_level(levels[0], [2.0, 2.0, 1.0, 1.0, 0.0, 0.0],
                          [10.0, 10.0, 20.0, 20.0, 30.0, 30.0])

        self._check_level(levels[1], [2.0, 1.5, 2.0, 2.33333333, 2.0],
                          [25.0, 30.0, 33.33333333, 30.0, 25.0])

        levels = c.trace(2.0)
        self.assertEqual(len(levels), 1)
        self._check_level(levels[0], [3.0, 2.0, 2.66666667, 2.0, 1.0, 0.0],
                          [10.0, 20.0, 30.0, 36.66666667, 30.0, 40.0])

        levels = c.trace(2.5)
        self.assertEqual(len(levels), 1)
        self._check_level(levels[0], [3.0, 2.5, 2.83333333, 2.0, 1.0, 0.5],
