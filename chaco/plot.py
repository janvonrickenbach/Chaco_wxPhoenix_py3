...
class Plot(DataView):
    ...
    auto_colors = List([
        "green", "lightgreen", "blue", "lightblue", "red", "pink", "darkgray",
        "silver"
    ])
    ...
    def plot(self,
        ...
        if plot_type in ("line", "scatter", "polygon", "bar", "filled_line"):
            ...
            if plot_type == 'bar':
                ...
                def custom_index_func(data_low, data_high, margin,
                                      tight_bounds):
                ...
                def custom_value_func(data_low, data_high, margin,
                                      tight_bounds):
            ...
            if len(data) != 3:
                ...
                if isinstance(colormap, AbstractColormap):
                    ...
                    raise ValueError("Unexpected colormap %r in plot()." %
                                     colormap)
    ...
    def candle_plot(self,
        ...
        if len(data) == 3:
            ...
            index, bar_min, center, bar_max = list(
                map(self._get_or_create_datasource, data))
            ...
            index, min, bar_min, bar_max, max = \
                            list(map(self._get_or_create_datasource, data))
    ...
    def hideplot(self, *names):
        ...
        """
        for renderer in itertools.chain(
                * [self.plots[name] for name in names]):
    ...
    def _color_mapper_changed(self):
        for plist in list(self.plots.values()):
            for plot in plist:
    ...
    def _data_changed(self, old, new):
        if old:
            old.on_trait_change(
                self._data_update_handler, "data_changed", remove=True)
        if new:
    ...
    def _handle_range_changed(self, name, old, new):
        ...
        for renderer in itertools.chain(*list(self.plots.values())):
